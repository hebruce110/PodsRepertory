...
@property (nonatomic, strong) ALButton *downloadButton;
...
- (void)setup {
    ...
    [_durationSlider setMaximumTrackImage:maxImage forState:UIControlStateNormal];
    [_durationSlider setMinimumTrackImage:minImage forState:UIControlStateNormal];
...
//        [_bottomBar addSubview:_volumeView];
    ...
    //下载按钮
    _downloadButton = [[ALButton alloc] init];
    [_downloadButton setImage:[UIImage imageNamed:@"movieDown.png"] forState:UIControlStateNormal];
    [_downloadButton setImage:[UIImage imageNamed:@"movieDown.png"] forState:UIControlStateHighlighted];
    ...
    [_bottomBar addSubview:_downloadButton];
...
- (void)downloadPressed:(UIButton *)button {
    UIAlertView *al = [[UIAlertView alloc]initWithTitle:@"开始下载..." message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
    [al show];
}
...
- (void)layoutSubviews {
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        self.seekBackwardButton.frame = CGRectMake(paddingFromBezel, self.barHeight/2 - seekHeight/2 + 1.f, seekWidth, seekHeight);
        self.playPauseButton.frame = CGRectMake(self.seekBackwardButton.frame.size.width + self.seekBackwardButton.frame.origin.x+paddingBetweenPlaybackButtons, self.barHeight/2 - playHeight/2, playWidth, playHeight);
        self.seekForwardButton.frame = CGRectMake(self.playPauseButton.frame.size.width + self.playPauseButton.frame.origin.x+paddingBetweenPlaybackButtons, self.barHeight/2 - seekHeight/2 + 1.f, seekWidth, seekHeight);
      
        self.timeElapsedLabel.frame = CGRectMake(self.seekForwardButton.frame.origin.x + self.seekForwardButton.frame.size.width + paddingBetweenButtons, 0, labelWidth, self.barHeight);
        ...
        self.downloadButton.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - fullscreenWidth - paddingFromBezel - fullscreenWidth , self.barHeight/2 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
        
        self.timeRemainingLabel.frame = CGRectMake(self.downloadButton.frame.origin.x - paddingBetweenButtons - labelWidth, 0, labelWidth, self.barHeight);
    }
